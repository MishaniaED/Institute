...
int     get_val(char a);        //char to int
char    back_to_char(int a);    //int to char
char*   dec_to_base(unsigned int dec_digit, int base, char(*str_digit)[BUFSIZ]);  // digit in base to string
int     base_to_dec(char str_digit[BUFSIZ], int base);                            //string to number in base
int     cmd_prc(char* cmd, unsigned int(*val_arr)[26][2], int* size);             //parse command and check errors
...
int     main(int argc, char** argv)
{
    ...
    if (argc == 4 && !(trace_file = fopen(argv[3], "w")))
    {
        perror("File error");
        fclose(input_file);
        exit(-6);
    ...
    while (fgets(string_buff, BUFSIZ, input_file))
    {
        for (i = 0; i < strlen(string_buff); i++)
        {
            ...
            {
                comm_buff[j] = '\0';
                error = cmd_prc(comm_buff, &val_arr, &size);
                if (error == -1)
                {
                ...
                if (argc == 4)
                {
                    if (error == 1) fprintf(trace_file, "Value assignment to %c from %c\n", comm_buff[0], comm_buff[3]);
                    else if (error == 2)
                        fprintf(trace_file, "Inversed value assignment to %c from %c\n", comm_buff[0], comm_buff[3]);
                    else if (error == 10 || error == 11 || error == 15 || error == 18 || error == 19)
                        fprintf(trace_file, "Value assignment to %c from operation %c %c %c\n", comm_buff[0], comm_buff[3], comm_buff[4], comm_buff[5]);
                    else if (error == 12 || error == 13 || error == 16 || error == 17)
                        fprintf(trace_file, "Value assignment to %c from operation %c %c%c %c\n", comm_buff[0], comm_buff[3], comm_buff[4], comm_buff[5], comm_buff[6]);
                    else if (error == 20)
                    {
                        int base;
                        char tmp[BUFSIZ];
...
{
    int     dec_digit = 0, i;
    int     length = strlen(str_digit), lb = 0, tmp = 1;
...
int cmd_prc(char* cmd, unsigned int (*val_arr)[26][2], int* size)
{
    int     flag = 1,base, k, i;
    char    num[BUFSIZ];
    char    read_str[6] = "READ(";
    char    write_str[7] = "WRITE(";
